...
public class ImageSpecific
{
	 ...
	 * @return
	 */
	public Bitmap imageSpecific(Bitmap bmp, Bitmap bmp2, int flag)
	{
		Bitmap bm = null;
		switch (flag)
		{
...
//			bm = sunshine(bmp);
	...
	private Bitmap oldRemeber(Bitmap bmp)
	{
		...
		Log.d("may", "used order_firsttime="+(end - start));
		return bitmap;
	}
	 ...
	 * @return
	 */
	private Bitmap overlay(Bitmap bmp, Bitmap bmp2)
	{
		long start = System.currentTimeMillis();
		int width = bmp.getWidth();
		int height = bmp.getHeight();
	 ...
	 * @return
	 */
	private Bitmap overlayAmeliorate(Bitmap bmp, Bitmap bmp2)
	{
		long start = System.currentTimeMillis();
		int width = bmp.getWidth();
		int height = bmp.getHeight();
		...
		long end = System.currentTimeMillis();
		Log.d("may", "overlayAmeliorate used order_firsttime="+(end - start));
		return bitmap;
	}
	 ...
	 * @return
	 */
	private Bitmap alphaLayer(Bitmap bmp, Bitmap frameBitmap)
	{
		int width = bmp.getWidth();
		int height = bmp.getHeight();
		Bitmap bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
